...
* am
*
* transform.c                     S. Paine rev. 2024 July 22 
*
*
 ...
 * OpenMP, if nested parallelism is enabled each recursive call
 * starts two new threads until either the maximum nesting depth
 * is reached, or the branch count exceeds the maximum number of
 ...
 *
 * (Update Nov 2023: Within L1 cache on Apple M1 Pro, the non-local
 * transforms are about twice as fast as the sequential ones.)
 */
...
void ft_benchmarks(void)
{
    ...
    printf("%24s  %8s  %8s  %12s\n",
            "function", "ngrid", "ncalls", "t[us]/call");
    printf("%24s  %8s  %8s  %12s\n",
            "--------", "-----", "------", "----------");
